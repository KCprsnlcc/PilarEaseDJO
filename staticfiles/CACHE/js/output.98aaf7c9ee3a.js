...
async function renderNotifications(page=1){const notifications=await fetchNotifications(page);if(notifications.length===0){loadMoreButton.style.display="none";seeAllButton.style.display="block";return;}
...
window.location.href=notification.link;});item.innerHTML=`
        ...
        `;notificationItems.appendChild(item);renderedNotifications.set(notification.id,notification);}});if(currentPage<totalPages){loadMoreButton.style.display="block";seeAllButton.style.display="none";}else{loadMoreButton.style.display="none";seeAllButton.style.display="block";}
